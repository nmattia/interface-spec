...
well_formed_forest(trees) =
  strictly_increasing([l | Labeled l _ ∈ trees]) ∧
  ∀ Labeled _ t ∈ trees. well_formed(t) ∧
  ∀ t ∈ trees. t ≠ Leaf _
....
...
....
check_delegation(NoDelegation) : Blob =
  return root_public_key
check_delegation(Delegation d) : Blob =
...
....
  ...
  ∀ path ∈ RS.paths. may_read_path(S, RS.sender, path)
...
may_read_path(S, _, ["request_status", Rid] · _) =
  if ∃ R ∈ dom(S.requests). hash_of_map(R) = Rid
...
state_tree(S) = {
  "time": S.system_time;
  "subnet": { subnet_id : { "public_key" : subnet_pk; "canister_ranges" : CBOR(subnet_canister_ranges) } | (subnet_id, subnet_pk, subnet_canister_ranges) ∈ subnets };
  "request_id": { hash_of_map(R): request_status_tree(S) | (R ↦ S) ∈ S.requests };
  "canister":
    { canister_id :
        { "module_hash" : SHA256(C.raw_module) | if C ≠ EmptyCanister } ∪
        { "controllers" : CBOR(S.controllers[canister_id]) } ∪
        { "metadata": { name: blob | (name, blob) ∈ C.public_custom_sections ∪ C.private_custom_sections } }
